...
<div class="wrap" id="of_container">
  ...
  <form id="of_form" method="post" action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" enctype="multipart/form-data" >
	...
	<button id ="of_save" type="button" class="button-primary"><?php _e('Save All Changes','kauri');?></button>
	</div><!--.info_bar--> 	
	...
	<button id ="of_save" type="submit" class="button-primary"><?php _e('Save All Changes','kauri');?></button>
	<button id ="of_reset" type="submit" class="button submit-button reset-button" ><?php _e('Options Reset','kauri');?></button>
	</div><!--.save_bar--> 
...
function of_admin_head() { 
	...
	$('#of_backup_button').live('click', function(){
		...
		var answer = confirm("<?php _e('Click OK to backup your current saved options.','kauri');?>")
	...
	$('#of_restore_button').live('click', function(){
		...
		var answer = confirm("<?php _e('Warning: All of your current options will be replaced with the data from your last backup! Proceed?','kauri');?>")
	...
	$('#of_reset').click(function() {
		var answer = confirm("<?php _e('Click OK to reset. All settings will be lost!','kauri');?>")
	...
	$('input.nag_input').click(function() {
		$('.section-nag_ignore ').fadeToggle('slow');
	});
	...
	foreach ($options as $value) {
		...
		case 'select':
			...
			foreach ($value['options'] as $select_ID => $option) {			
				$output .= '<option id="' . $select_ID . '" value="'.$option.'" ' . selected($data[$value['id']], $option, false) . ' />'.
				$option.'</option>';	 
			 } 
		...
		case 'select_category':
			...
			foreach ($value['options'] as $select_ID => $option) {			
				$output .= '<option id="' . $select_ID . '" value="'.$option[1].'" ' . selected($data[$value['id']], $option[1], false) . ' />'.
				$option[0].'</option>';	 
			};
		...
		case "radio":
			 ...
			 foreach($value['options'] as $option=>$name) {
				$output .= '<input class="of-input of-radio" name="'.$value['id'].'" type="radio" value="'.$option.'" ' . checked($data[$value['id']], $option, false) . ' /><label class="of-radio-label">'.$name.'</label><br/>';				
			}	 
		break;
		case 'checkbox':
		...
		case 'typography':
			...
			if(isset($typography_stored['face-heading'])) {
			
				$output .= '<div class="select_wrapper typography-face">';
				$output .= '<select class="of-typography of-typography-face-heading select" name="'.$value['id'].'[face-heading]" id="'. $value['id'].'_face">';
				...
				foreach ($faces as $i=>$face_heading) {
					$output .= '<option value="'. $i .'" ' . selected($typography_stored['face-heading'], $i, false) . '>'. $face_heading .'</option>';
				}			
		...
		case "nag_ignore":
			...
			if ( !$data[$value['id']] ) {
				$output .= '<div class="nag">';
				$output .= $message ;

				$output .= '<p><input type="hidden" class="aq-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="0"/>';
				$output .= '<input type="checkbox" class="nag_input" name="'.$value['id'].'" id="'. $value['id'] .'" value="1" '. checked($data[$value['id']], 1, false) .' />';
				$output .= '<label for="'. $value['id'] .'"></label></p>';
				$output .= '</div>';
			}
		...
		$output .= '<p><strong>'. __('Last Backup : ','kauri').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
