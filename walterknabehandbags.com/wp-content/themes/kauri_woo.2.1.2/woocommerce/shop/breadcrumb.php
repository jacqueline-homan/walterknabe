...
if( !$home ) $home = get_bloginfo('name');
...
if ( (!is_home() && !is_front_page() && !(is_post_type_archive() && get_option('page_on_front')==woocommerce_get_page_id('shop'))) || is_paged() ) :
	...
	elseif ( is_tax('product_tag') ) :
		...
		$queried_object = $wp_query->get_queried_object();
		echo $prepend . $before . __('Products tagged &ldquo;', 'kauri') . $queried_object->name . '&rdquo;' . $after;
	...
	elseif ( is_post_type_archive('product') && get_option('page_on_front') !== woocommerce_get_page_id('shop') ) :
		...
		if (is_search()) :
			...
			echo $before . '<a href="' . get_post_type_archive_link('product') . '">' . $_name . '</a>' . $delimiter . __('Search results for &ldquo;', 'kauri') . get_search_query() . '&rdquo;' . $after;
	...
	elseif ( is_search() ) :
		...
		echo $before . __('Search results for &ldquo;', 'kauri') . get_search_query() . '&rdquo;' . $after;
	
	elseif ( is_tag() ) :
	
			echo $before . __('Posts tagged &ldquo;', 'kauri') . single_tag_title('', false) . '&rdquo;' . $after;
	
	elseif ( is_author() ) :
	
		$userdata = get_userdata($author);
		echo $before . __('Author:', 'kauri') . ' ' . $userdata->display_name . $after;
