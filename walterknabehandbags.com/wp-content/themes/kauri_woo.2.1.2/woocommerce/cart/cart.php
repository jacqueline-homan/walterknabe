...
<table class="shop_table cart" cellspacing="0">
	<thead>
		<tr>
			...
			<th class="product-name"><span><?php _e('Product Name', 'kauri'); ?></span></th>
			<th class="product-price"><span><?php _e('Unit Price', 'kauri'); ?></span></th>
			<th class="product-quantity"><?php _e('Quantity', 'kauri'); ?></th>
			<th class="product-subtotal"><?php _e('Unit total', 'kauri'); ?></th>
		</tr>
	</thead>
	<tbody>
		...
		if ( sizeof( $woocommerce->cart->get_cart() ) > 0 ) : 
			foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $values ) :
				...
				if ( $_product->exists() && $values['quantity'] > 0 ) :
					?>
					<tr>
						<td class="product-remove"><a href="<?php echo esc_url( $woocommerce->cart->get_remove_url($cart_item_key) ); ?>" class="remove" title="<?php _e('Remove this item', 'kauri'); ?>">&times;</a></td>
						...
						<td class="product-name">
							...
							<?php
								...
								echo $woocommerce->cart->get_item_data( $values );
                   				...
                   				if ($_product->backorders_require_notification() && $_product->get_total_stock()<1) echo '<p class="backorder_notification">'.__('Available on backorder.', 'kauri').'</p>';
							?>
						</td>
		...
		<tr>
			<td colspan="6" class="actions">
				<?php if ( get_option( 'woocommerce_enable_coupons' ) == 'yes' ) { ?>
					<div class="coupon">
						<label for="coupon_code"><?php _e('Coupon', 'kauri'); ?>:</label> <input name="coupon_code" class="input-text" id="coupon_code" value="" /> <input type="submit" class="button" name="apply_coupon" value="<?php _e('Apply Coupon', 'kauri'); ?>" />
					</div>
				<?php } ?>
				<?php $woocommerce->nonce_field('cart') ?>
				<input type="submit" class="button" name="update_cart" value="<?php _e('Update Cart', 'kauri'); ?>" /> <a href="<?php echo esc_url( $woocommerce->cart->get_checkout_url() ); ?>" class="checkout-button button alt"><?php _e('Proceed to Checkout &rarr;', 'kauri'); ?></a>
