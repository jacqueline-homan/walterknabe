<?php
function get_post_image ($post_id=0, $width=0, $height=0, $alt='', $img_script='') {
	global $wpdb;
	if($post_id > 0) {
			...
			if(count($frags)) {
			  ...
			  if(strlen($the_image_src)) {
				   if(strlen($img_script)) {
					    ...
					    $the_image = '<img class="thumbnail" alt="'.$alt.'" src="' . $img_script . $the_image_src . '" />';
				   }
				   else {
					    $the_image = '<img class="thumbnail" alt="'.$alt.'" src="' . $the_image_src . '" width="' . $width . '" height="' . $height . '" />';
				   }
			  }
			  return $the_image;
		 }
...
function catch_that_image() {
  ...
  if(empty($first_img)){ //Defines a default image
    //$first_img = "/images/clearline.gif";
  }
  return $first_img;
}
...
function childtheme_menus() { ?>
        ...
        <div id="category-menu" class="menu" style="display: none;">
...
function remove_footer() {
	remove_action('thematic_footer', 'thematic_footer');
}
add_action('init', 'remove_footer');
  ...
  while ( have_posts() ) : the_post() ?>
			...
			?>">
				<div class="entry-content">
					...
					<?= get_post_image (get_the_id(), '', '', get_the_title(), '' .get_bloginfo('template_url') .'/scripts/timthumb.php?zc=1&amp;w=125&amp;h=125&amp;src='); ?>
					<? /*
                    <img class="thumbnail" src="<?php if(get_post_meta($post->ID, 'thumbnail', $single = true)){echo get_post_meta($post->ID, 'thumbnail', $single = true);} else{bloginfo('url'); echo "/wp-content/themes/gallery/images/thumbnail-default.jpg";} ?>" width="125" height="125" alt="<?php echo the_title() ?>" />*/ ?>
					</a>
				</div>
			</div><!-- .post -->
		...
		?>
			...
			?>">
			  <div class="entry-content">
				...
				$content = str_replace('<hr />', '</div><div id="homeright">', $content);
          ...
          <?php if($video==1) {echo p75GetVideo($post->ID); }
              ...
              <?php }else{ ?>
				...
				if($topimg) {
					echo '<a href="'.$topimg.'"><img src="'.get_bloginfo('template_url').'/scripts/timthumb.php?src='.$topimg.'&w=500&h=2500&zc=1"></a>';
				} else {
					echo '<img src="/wp-content/themes/gallery/images/full-image-default.jpg">';
				}
				?>
				<? /*
...
function remove_comments() {
  remove_action('thematic_commentstemplate', 'thematic_comments_template');
}
